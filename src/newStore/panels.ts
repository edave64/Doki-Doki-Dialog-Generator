...
import type { BackgroundLookup, IAssetSwitch } from '@/store/content';
...
export const ui = new (class Panels {
	private _list: Raw<Panel>[] = [];
	...
	duplicatePanel(panel: Panel) {
		...
		const idx = this._list.indexOf(panel);
		undoAble(
			() => void this._list.splice(idx + 1, 0, newPanel),
			() => void this._list.splice(idx + 1, 1)
		);
		return newPanel;
	}
	...
	deletePanel(panel: Panel) {
		...
		undoAble(
			() => void this._list.splice(idx, 1),
			() => void this._list.splice(idx, 0, panel)
		);
	}
	...
	movePanel(panel: Panel, delta: number) {
		...
		undoAble(
			() => {
				...
				this._list.splice(idx + delta, 0, panel);
			},
			() => {
				this._list.splice(idx + delta, 1);
				this._list.splice(idx, 0, panel);
			}
		);
	}

	fixContentPackRemoval(oldContent: ContentPack<IAssetSwitch>) {
		for (const panel of Object.values(this._list)) {
...
export class PanelBackground extends HasSpriteFilters {
	...
	public fixContentPackRemoval(oldContent: ContentPack<IAssetSwitch>) {
		...
		const newBackground = (content.getBackgrounds as BackgroundLookup).get(
			this.current
		);
